...
bot.on("message:text", async (ctx) => {
  ...
  try {
    const { category, model } = await moderateMessage(msg.text, MISTRAL_API_KEY);

    if (category === "severe") {
      await ctx.deleteMessage();

      const warning = await ctx.reply(
        `${WARNING_MESSAGE}\n— <i>${msg.from.first_name}</i> · <code>${model}</code>`,
        { parse_mode: "HTML" }
      );
      ...
      console.log(
        `[moderated] chat=${ctx.chat.id} user=${msg.from.id} model=${model}`
      );
    }
  } catch (err) {
